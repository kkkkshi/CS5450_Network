...
typedef struct state_t{
	...
	int state;                /* current state-machine state (see enum below)        */
	volatile uint8_t seqnum;  /* next seq num to send (sender) / to expect (receiver)*/
	int winsize;              /* current window size - the 'n' in go-back-n          */
	struct sockaddr_in peer;  /* address of the other end                            */
	socklen_t peerlen;
